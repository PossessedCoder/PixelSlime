...
class Levels(BaseWindow):
    ...
    def __init__(self):
        ...
        self.back_button.set_hovered_view(load_media(Images.BACK), background_color=(69, 69, 69),
                                          border_radius=15, scale_x=1.03, scale_y=1.03)
        self.back_button.set_not_hovered_view(load_media(Images.BACK), background_color=(78, 78, 78),
                                              border_radius=25)
        self.user_levels_list_button = Button(self.get_width() - self._but_w * 3, self.get_height() - self._but_h,
                                              self._but_w * 3, self._but_h)
        self.user_levels_list_button.set_hovered_view(load_media(Images.BACK), background_color=(69, 69, 69),
                                                      border_radius=15, scale_x=1.03, scale_y=1.03)
        self.user_levels_list_button.set_not_hovered_view(load_media(Images.BACK), background_color=(78, 78, 78),
                                                          border_radius=25)
        ...
        for x in range(self._but_w, self.get_width() - self._but_w, self._but_w + self._but_w // 2):
            for y in range(self._but_h, self.get_height() - self._but_h,
                ...
                a.set_hovered_view(load_media(Images.LEVELS_PREVIEW), background_color=(69, 69, 69),
                                   border_radius=15, scale_x=1.03, scale_y=1.03)
                a.set_not_hovered_view(load_media(Images.LEVELS_PREVIEW), background_color=(78, 78, 78),
                                       border_radius=25)
